#ifndef PREDICACHE_PREDICACHEPP_HPP
#define PREDICACHE_PREDICACHEPP_HPP
...
// Custom reimplementation of memcmp
inline int cust_memcmp(const void* ptr1, const void* ptr2, size_t num) {
...
struct BTreeNode : public BTreeNodeHeader {
   ...
   BTreeNode(bool isLeaf) : BTreeNodeHeader(isLeaf) { setDirty(true); }
   ...
   u16 lowerBound(span<u8> skey, bool& foundExactOut) {
      ...
      // check prefix
      int cmp = cust_memcmp(skey.data(), getPrefix(), min(skey.size(), prefixLen));
      ...
      while (lower < upper) {
         ...
         if (keyHead < slot[mid].head) {
            ...
            int cmp = cust_memcmp(key, getKey(mid), min(keyLen, slot[mid].keyLen));
            if (cmp < 0) {
               upper = mid;
            } else if (cmp > 0) {
               lower = mid + 1;
...
#endif // PREDICACHE_PREDICACHEPP_HPP