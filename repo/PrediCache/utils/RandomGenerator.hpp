...
// -------------------------------------------------------------------------------------
inline MersenneTwister::MersenneTwister(uint64_t seed) : mti(NN + 1)
{
   init(seed + (mt_counter++));
}
// -------------------------------------------------------------------------------------
inline void MersenneTwister::init(uint64_t seed)
{
   mt[0] = seed;
...
// -------------------------------------------------------------------------------------
inline uint64_t MersenneTwister::rnd()
{
   int i;
   uint64_t x;
...
// -------------------------------------------------------------------------------------
inline void RandomGenerator::getRandString(u8* dst, u64 size)
{
...
class XOrshift128p {
   uint64_t xorshift_state[2] = {0,0};

public:
   void init() {
      std::random_device rd; // Strong seed
      xorshift_state[0] = static_cast<u64>(rd()) << 32;
      xorshift_state[0] |= rd();
      xorshift_state[1] = static_cast<u64>(rd()) << 32;
      xorshift_state[1] |= rd();
   }

   uint64_t rnd() {
      if (xorshift_state[0] == 0 && xorshift_state[1] == 0) [[unlikely]]
         init();
      ...
      xorshift_state[0] = s0;
      s1 ^= s1 << 23; // a
      xorshift_state[1] = s1 ^ s0 ^ (s1 >> 18) ^ (s0 >> 5); // b, c
      return result;
   }
};

static thread_local XOrshift128p xorshift128p;
...
public:
   ...
   static u64 getRandU64(u64 min, u64 max)
   {
      u64 rand = min + (xorshift128p.rnd() % (max - min));
      assert(rand < max);
      assert(rand >= min);
      return rand;
   }
   static u64 getRandU64() { return xorshift128p.rnd(); }
   template <typename T>
   static inline T getRand(T min, T max)
   {
      u64 rand = getRandU64(min, max);
...
static thread_local FastRandomGenerator fast_rng;
